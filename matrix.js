...
const showResult2D = (title, containerId, dataArray) => {
    ...
    for (let i = 0; i < dataArray.length; i++) {
        ...
        for (let j = 0; j < dataArray[i].length; j++) {
            let td = document.createElement('td');
            let span = document.createElement('span');
            span.innerHTML = dataArray[i][j]; // Set the value from the result array
            td.appendChild(span);
            tr.appendChild(td);
        }
        table.appendChild(tr);
...
function performOperation(operation) {
    ...
    if (operation === 'add') {
        if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
            showResult2D("Error, incompatible matrix dimensions for adding.", "matrix3", []);
        }
        else {
            showResult2D("Matrix 1 and 2 Added", "matrix3", addMatrices(matrix1, matrix2))
        };
    ...
    if (operation === 'subtract') {
        if (matrix1.length !== matrix2.length || matrix1[0].length !== matrix2[0].length) {
            showResult2D("Error, incompatible matrix dimensions for subtracting.", "matrix3", []);
        }
        else {
            showResult2D("Matrix 1 and 2 Subtracted", "matrix3", subtractMatrices(matrix1, matrix2))
        };
    ...
    if (operation === 'multiply') {
        if (matrix1[0].length !== matrix2.length) {
            showResult2D("Error, incompatible matrix dimensions for multiplying.", "matrix3", []);
        }
        else {
            showResult2D("Matrix 1 and 2 Multiplied", "matrix3", multiplyMatrices(matrix1, matrix2))
        };
